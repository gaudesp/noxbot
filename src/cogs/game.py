...
class GameCog(commands.Cog):
  ...
  async def follow(self, interaction: discord.Interaction, game: str, channel: discord.TextChannel) -> None:
    ...
    if game_exist:
      await interaction.followup.send(self.bot.i18n.translate("cogs.game.commands.nx_follow.messages.exist", locale, game_name=game_exist.name, channel=game_exist.channel), ephemeral=True)
      return

    game_found = await self.steam_service.get_game_name(game)
    if not game_found:
      await interaction.followup.send(self.bot.i18n.translate("cogs.game.commands.nx_follow.messages.not_found", locale), ephemeral=True)
      return
    
    new_game = await self.game_service.add_game(game, interaction.guild.id, channel.id, game_found)
    await interaction.followup.send(self.bot.i18n.translate("cogs.game.commands.nx_follow.messages.success", locale, game_name=new_game.name, channel=new_game.channel), ephemeral=True)
  ...
  async def unfollow_game(self, interaction: discord.Interaction, game: str) -> None:
    ...
    if not game_followed:
      await interaction.followup.send(self.bot.i18n.translate("cogs.game.commands.nx_unfollow.messages.not_followed", locale), ephemeral=True)
      return
    
    await interaction.followup.send(self.bot.i18n.translate("cogs.game.commands.nx_unfollow.messages.success", locale, game_name=game_followed.name, channel=game_followed.channel), ephemeral=True)
  ...
  async def list_games(self, interaction: discord.Interaction) -> None:
    ...
    if not games_followed:
      await interaction.followup.send(self.bot.i18n.translate("cogs.game.commands.nx_list.messages.not_followed", locale), ephemeral=True)
      return
    
    await interaction.followup.send(''.join([self.bot.i18n.translate("cogs.game.commands.nx_list.messages.success", locale, game_name=game.name, app_id=game.id, channel=game.channel) for game in games_followed]), ephemeral=True)
  ...
  async def reset_games(self, interaction: discord.Interaction) -> None:
    ...
    if not games_followed:
      await interaction.followup.send(self.bot.i18n.translate("cogs.game.commands.nx_reset.messages.not_followed", locale), ephemeral=True)
      return
    
    await interaction.followup.send(self.bot.i18n.translate("cogs.game.commands.nx_reset.messages.success", locale, games=games_followed), ephemeral=True)
