...
class PublishCommands(commands.Cog):
  ...
  @ensure_server
  async def publish(self, interaction: discord.Interaction, game_id: str) -> None:
    await interaction.response.defer(ephemeral=True, thinking=True)

    find_followed_game = await self.bot.database.execute(select(FollowedGame).options(joinedload(FollowedGame.game).joinedload(Game.news)).where(FollowedGame.game_id == game_id, FollowedGame.server_id == self.server.id))
